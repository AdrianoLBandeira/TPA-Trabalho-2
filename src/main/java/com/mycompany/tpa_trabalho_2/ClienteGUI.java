...
public class ClienteGUI {
    ...
    private GeradorDeArquivosDeClientes geradorDeArquivos;

    public ClienteGUI() {
        bufferDeClientes = new BufferDeClientes();
        listaClientes = new ArrayList<>();
        geradorDeArquivos = new GeradorDeArquivosDeClientes();
        criarInterface();
    }
    ...
    private void criarInterface() {
        ...
        JButton btnGerarLista = new JButton("Gerar Lista de Clientes");
        ...
        painelControle.add(btnGerarLista);
        ...
        btnGerarLista.addActionListener(e -> solicitarDadosGeracaoArquivo());

        frame.setVisible(true);
    }
    private void solicitarDadosGeracaoArquivo() {
        JTextField nomeArquivoField = new JTextField();
        JTextField quantidadeClientesField = new JTextField();

        Object[] campos = {
            "Nome do Arquivo:", nomeArquivoField,
            "Quantidade de Clientes:", quantidadeClientesField
        };

        int option = JOptionPane.showConfirmDialog(null, campos, "Gerar Lista de Clientes", JOptionPane.OK_CANCEL_OPTION);
        if (option == JOptionPane.OK_OPTION) {
            try {
                String nomeArquivo = nomeArquivoField.getText().trim();
                int quantidadeClientes = Integer.parseInt(quantidadeClientesField.getText().trim());
                if (!nomeArquivo.isEmpty() && quantidadeClientes > 0) {
                    gerarListaDeClientes(nomeArquivo, quantidadeClientes);
                } else {
                    JOptionPane.showMessageDialog(null, "Por favor, insira um nome de arquivo válido e uma quantidade positiva.");
                }
            } catch (NumberFormatException e) {
                JOptionPane.showMessageDialog(null, "A quantidade de clientes deve ser um número válido.");
            }
        }
    }

    private void gerarListaDeClientes(String nomeArquivo, int quantidadeClientes) {
        geradorDeArquivos.gerarArquivoClientes(nomeArquivo, quantidadeClientes);
        JOptionPane.showMessageDialog(null, "Arquivo de clientes gerado com sucesso!");
    }
